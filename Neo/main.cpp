...
int main(int argc, wchar_t **argv){
	...
	try {
		...
		warningTexture = new KLGLTexture("common/textures/warning.png");
		startupTexture = new KLGLTexture("common/textures/startup.png");
		creditsTexture = new KLGLTexture("common/textures/credits.png");

		// Init sound
		audio = new Audio();
		audio->loadSound("common/sounds/startup.ogg", 10, FMOD_LOOP_OFF);
		audio->loadSound("common/sounds/8bp123-01-jellica-dennye.mp3", 0, FMOD_LOOP_NORMAL);
		...
		// UI
		dialog = new UIDialog(gc, "common/textures/UI_Interface.png");
		...
		numberFont = new KLGLFont(charmapTexture2->gltexture, charmapTexture2->width, charmapTexture2->height, 5, 5, '0');
		...
		if (t0-t2 >= CLOCKS_PER_SEC){
			...
			case GameMode::_MAPLOAD:										// ! Threaded map loader :D
				...
				try{
					...
					audio->system->playSound(FMOD_CHANNEL_FREE, audio->sound[4], false, &audio->channel[1]);
					audio->channel[0]->setVolume(0.5f);
					audio->channel[1]->setVolume(0.15f);
			...
			case GameMode::_CREDITS:
				...
				if (scrollPos == INT_MAX){
					scrollPos = gc->buffer.height;
				}else if(scrollPos < -2400){
					mode = -1;
				}
				scrollPos -= (frame%3 == 2);
				gc->Blit2D(creditsTexture, 0, scrollPos);
				gc->OrthogonalEnd();
				gc->Swap();
				break;
			default:
